...
switch choFlag
    ...
    case 3 % single pulse triggered average
        pulses = bsxfun(@times, pulses, sigma);
        pulses = bsxfun(@plus, pulses, mu);
        rbar = pulseSTASingle(pulses, bsxfun(@minus, rtrue, mean(rtrue)), ptimes, nk);
        sigma = 1;
        mu = 0;
        ptaTime = ptaTime(:,1);
    otherwise
